import type { Track, Album, Artist, Genre, Playlist, SearchResult } from './types'
...
export async function getTrack(id: string): Promise<Track | null> {
  try { const data = await fetchJson<any>(`${DEEZER_API}/track/${id}`); return mapTrack(data) }
  catch { return null }
}
...
export async function getAlbum(id: string): Promise<{ album: Album; tracks: Track[] } | null> {
  try {
    ...
    return { album: mapAlbum(data), tracks: (data.tracks?.data || data.tracks || []).map(mapTrack) }
  } catch { return null }
}

export async function getArtist(id: string): Promise<Artist | null> {
  try { const data = await fetchJson<any>(`${DEEZER_API}/artist/${id}`); return mapArtist(data) }
  catch { return null }
}

export async function getArtistTopTracks(id: string, limit = 10): Promise<Track[]> {
  try { const data = await fetchJson<any>(`${DEEZER_API}/artist/${id}/top?limit=${limit}`); return (data.data || []).map(mapTrack) }
  catch { return [] }
}
...
export async function getGenres(): Promise<Genre[]> {
  ...
  return (data.data || []).map((item: any) => ({ id: String(item.id), name: item.name, image: item.picture_medium || item.picture || '' }))
}
...
export async function getArtistAlbums(id: string, limit = 10): Promise<Album[]> {
  try { const data = await fetchJson<any>(`${DEEZER_API}/artist/${id}/albums?limit=${limit}`); return (data.data || []).map(mapAlbum) }
  catch { return [] }
}
